...
public class BinarySearchTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
    ...
    protected void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
        ...
        // Update parent's parent (if it exists)
        Node<T> grandparent = parent.up;
        if (grandparent != null) {
            // Determine if the parent was a left or right child of the grandparent
            if (parent.isRightChild()) {
                grandparent.down[1] = child; // Set child as the new right child of the grandparent
            } else {
                grandparent.down[0] = child; // Set child as the new left child of the grandparent
            }
        } else {
            // If there's no grandparent, update the root to be the child
            root = child;
        }

        // Check if the child is a right child of the parent
        if (child.isRightChild()) {

            // Update parent's right child to be child's left child (B)
            parent.down[1] = child.down[0];
            if (child.down[0] != null) {
                // Update child's left child's parent reference to be the parent
                child.down[0].up = parent;
            }

            // Update child's left child to be the parent (A)
            child.down[0] = parent;
        }
        else {

            // Update parent's left child to be child's right child (B)
            parent.down[0] = child.down[1];
            if (child.down[1] != null) {
                // Update child's right child's parent reference to be the parent
                child.down[1].up = parent;
            }

            // Update child's right child to be the parent (A)
            child.down[1] = parent;
        }
    }
    ...
    public static boolean test1() {
        ...
        // Perform a right rotation at the root node (banana)
        BinarySearchTree.Node<String> root = tree.root;
        BinarySearchTree.Node<String> rightChild = root.down[1];
        tree.rotate(rightChild, root);

        // Check if the right rotation was performed correctly and returns a boolean statement
        return (tree.toInOrderString().equals("[ apple, banana, cherry, date, fig, grape, kiwi ]"));
    ...
    public static boolean test2() {
        ...
        // Perform a right rotation at the root node (10)
        BinarySearchTree.Node<Integer> root = tree.root;
        BinarySearchTree.Node<Integer> rightChild = root.down[1];
        tree.rotate(rightChild, root);

        // Check if the right rotation was performed correctly and returns a boolean statement
        return tree.toInOrderString().equals("[ 2, 5, 7, 10, 12, 15, 20 ]");
    }
    ...
    public static boolean test3() {
        ...
        // Perform a left rotation at the root node (8)
        BinarySearchTree.Node<Integer> root = tree.root;
        BinarySearchTree.Node<Integer> leftChild = root.down[0];
        tree.rotate(leftChild, root);

        // Check if the left rotation was performed correctly and returns a boolean statement
        return tree.toInOrderString().equals("[ 2, 4, 6, 8, 10, 12, 14 ]");
    }
